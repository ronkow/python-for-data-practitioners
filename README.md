...
Among the optional features, be sure to select `pip` which we will use to install Jupyterlab. `Documentation` provides a link to the complete documentation on the Python website as well as local access to the documentation written within the code of all the modules you install. `tcl/tk` (tool command language/toolkit) is for Python's tkinter module, used to build graphical user interfaces. If you do not wish to use IDLE, you can uncheck `tcl/tk and IDLE`. `Python test suite` is for core developers of the Python language and `py launcher` helps you to run different versions of Python installed on your system. For this tutorial, we will select `Documentation`, `pip` and `tcl/tk and IDLE`.
...
When you write a Python program, there may be functionalities in your program that you do not need to implement yourself if there are publicly available packages for these functionalities. For instance, you may need to plot a bar chart in your data modelling program. The easiest way to plot a bar chart is to use the popular data visualisation package called Matplotlib. You just need to install Matplotlib (using `pip`) and use the `import` keyword in your code to import the required modules from Matplotlib to your code. Your program now depends on Matplotlib. Some day, if you decide to share your data modelling program with someone, the person will need to install Matplotlib before he or she can run your code. <!-- you import become your program's dependencies.--> Matplotlib also depends on other packages. These other packages also depends on other packages. It is therefore not unusual for a package to have so many dependencies.
