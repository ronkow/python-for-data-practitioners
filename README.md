...
Among the optional features, be sure to select `pip` which we will use to install Jupyterlab. `Documentation` provides a link to the documention on the Python website as well as documentation you can access locally which I find useful. `tcl/tk` (tool command language/toolkit) is for Python's tkinter module, used to build graphical user interfaces. If you do not wish to use IDLE, you can uncheck `tcl/tk and IDLE`. `Python test suite` is for core developers of the Python language so you can uncheck this option. `py launcher` helps you to run different versions of Python installed on your system. We do not need this now so you can uncheck this option. For this tutorial, we will select `Documentation`, `pip` and `tcl/tk and IDLE`.
